...
async function verifyPreferencesFrontend() {
  console.log('\n=== PREFERENCES FRONTEND VERIFICATION ===\n');
  ...
  try {
    ...
    try {
      const updateResponse = await api.put('/users/onboarding/preferences', { data: testData });
      console.log('✓ Preferences update response:', updateResponse.data);
      ...
      console.error('✗ Preferences update failed:', error.response?.data || error.message);
      throw error;
    }
    ...
    try {
      ...
      const preferences = finalStatus.data.data.sections.preferences;
      if (!preferences) {
        throw new Error('Preferences section missing from status');
      }
      if (!preferences.completed) {
        throw new Error('Preferences section not marked as complete');
      }
...
verifyPreferencesFrontend().catch(console.error); 